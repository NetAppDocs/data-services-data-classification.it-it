...
[role="lead"]
Utilizza l'API REST di NetApp Data Classification per accedere alle stesse funzionalità disponibili nell'interfaccia utente web.
...
+
Ad esempio, puoi individuare file _senza_ specifiche informazioni di identificazione personale (PII) (funzionalità non disponibile nell'interfaccia utente). Puoi anche escludere campi specifici dall'operazione di esportazione.
...
Per accedere a Swagger, ti serve l'indirizzo IP della tua istanza di Data Classification. Per una deployment cloud, usa l'indirizzo IP pubblico. Poi devi accedere a questo endpoint:
...
.Richiesta API
Devi ottenere tutti i campi e le opzioni rilevanti per visualizzare tutti i filtri nella scheda Investigation.
...
----
Utilizza la risposta nei parametri della richiesta per filtrare i file che vuoi copiare.

È possibile applicare un'azione a più elementi.  I tipi di azioni supportati includono: sposta, elimina e copia.

Crea l'azione di copia:

.Richiesta API
Questa API ti permette di creare più azioni.
