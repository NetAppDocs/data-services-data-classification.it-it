...
[role="lead"]
NetApp Data Classification ti permette di salvare le query di ricerca e creare filtri personalizzati per trovare i dati nella pagina Indagini. Puoi usare query predefinite oppure creare query e criteri salvati per agire sui dati corrispondenti.
...
Puoi convertire le query salvate in **policy**. Mentre le query filtrano i dati, le policy ti permettono di agire sui dati. Con una policy, puoi eliminare i dati rilevati o inviare aggiornamenti via email su di essi.
...
Per visualizzare i risultati delle query salvate nella pagina Indagini, seleziona il pulsante image:button-gallery-options.gif["Pulsante Altro"] per una ricerca, poi seleziona *Indaga sui risultati*.
...
Puoi creare query salvate personalizzate che restituiscono risultati specifici per la tua organizzazione. I risultati vengono restituiti per tutti i file e le directory (condivisioni e cartelle) che corrispondono ai criteri di ricerca.
...
. Una volta configurate tutte le caratteristiche del filtro, seleziona *Salva query*.
+
...
Una volta creata la ricerca o la policy, puoi visualizzarla nella scheda **Query salvate**.
...
Non puoi modificare le query salvate predefinite o i filtri di una query salvata esistente. Invece, visualizza i risultati dell'investigazione della query salvata, cambia i filtri e salva il risultato come nuova query o criterio.
...
. Apporta modifiche ai campi nome e descrizione.
+
Puoi opzionalmente convertire la query in una policy o convertire la policy in una query salvata. Attiva o disattiva l'opzione *Esegui come policy* secondo necessità. .. Se stai convertendo la query in una policy, scegli se *Eliminare definitivamente* o *Inviare aggiornamenti via email*. Se scegli gli aggiornamenti via email, puoi inviare i risultati della query a _tutti_ gli utenti della Console ogni giorno, settimana o mese. In alternativa, puoi inviare la notifica a un indirizzo email specifico con la stessa frequenza.
...
+
File contenenti informazioni personali o dati personali sensibili, modificati più di sette anni fa
...
* **Protezione - Basso**
+
File a cui non hai avuto accesso per più di tre anni

* **Protezione - Media**
+
File o colonne di database che contengono identificativi di dati personali, inclusi numeri di identificazione, codici fiscali, numeri di patente di guida, identificativi medici o numeri di passaporto
