...
+
** Data Classification non può estrarre le autorizzazioni o l'“ora dell’ultimo accesso” dai sistemi 7-Mode.
...
NOTE: La pagina di panoramica del sistema include un'opzione *Scansione senza permessi di scrittura* disabilitata per impostazione predefinita. Se Data Classification non dispone dei permessi appropriati, non eseguirà la scansione dei file perché Data Classification non può ripristinare l'ora dell'ultimo accesso al timestamp originale. Puoi ignorare questa impostazione utilizzando l'opzione *Scansione senza permessi di scrittura*. Per saperne di più su questa opzione, vedi link:reference-collected-metadata.html#last-accessed-time-timestamp["Metadati raccolti da fonti di dati nella classificazione dei dati"].
...
NOTE: I nuovi volumi aggiunti al sistema vengono scansionati automaticamente solo se hai abilitato la scansione per tutti i volumi. Se hai abilitato la scansione solo su determinati volumi, devi abilitarla manualmente per il volume appena aggiunto.
