...
[role="lead"]
NetApp Data Classification è un servizio di governance dei dati disponibile in NetApp Console che analizza le origini dati locali e cloud per mappare e classificare i dati e identificare le informazioni private. Ti aiuta a ridurre i rischi per la sicurezza e la conformità, abbassare i costi di storage e supportare i progetti di migrazione dei dati.


IMPORTANT: A partire dalla versione 1.31, Data Classification è disponibile come funzionalità principale all'interno della NetApp Console. Non c'è alcun costo aggiuntivo. Non è richiesta alcuna licenza o abbonamento per Data Classification. + Se hai utilizzato la versione legacy 1.30 o precedente, quella versione sarà disponibile fino alla scadenza del tuo abbonamento.
...
La NetApp Console offre una gestione centralizzata dei servizi di storage e dati NetApp in ambienti on-premises e cloud su scala aziendale. La Console è necessaria per accedere e utilizzare i servizi dati NetApp. Come interfaccia di gestione, ti permette di gestire molte risorse di storage da un'unica interfaccia. Gli amministratori della Console possono controllare l'accesso allo storage e ai servizi per tutti i sistemi all'interno dell'azienda.
...
* Identifica le informazioni che consentono l'identificazione personale (PII).
...
NetApp Data Classification fornisce strumenti per aiutarti a ridurre il costo totale di proprietà (TCO) dello storage. Puoi usare NetApp Data Classification per:
...
I costi di trasferimento dati dipendono dalla tua configurazione. Se l'istanza di Data Classification e la fonte dei dati si trovano nella stessa availability zone e regione, allora non ci sono costi di trasferimento dati. Ma se la fonte dei dati, come un sistema Cloud Volumes ONTAP, si trova in una availability zone o regione diversa, allora il tuo cloud provider ti addebiterà i costi di trasferimento dati. Consulta questi link per maggiori dettagli:
...
* In AWS, Data Classification viene eseguito su un https://aws.amazon.com/ec2/instance-types/m6i/["istanza m6i.4xlarge"^] con un disco GP2 da 500 GiB. L'immagine del sistema operativo è Amazon Linux 2.
...
* **Le scansioni complete** offrono una scansione approfondita dei tuoi dati. Una scansione completa include una scansione di sola mappatura e la classificazione dei dati all'interno dei file.


Per una ripartizione delle differenze tra scansioni solo mappa e scansioni complete, vedere link:task-scanning-overview.html["Qual è la differenza tra le scansioni di mappatura e di classificazione?"].
