...
summary: Con la classificazione dei dati, è possibile ridurre i costi di archiviazione identificando dati obsoleti, file duplicati e file di grandi dimensioni, per poi rimuoverli o spostarli in storage a oggetti 
---
= Visualizza i dettagli di governance relativi ai dati organizzativi con NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. All'interno di ogni riquadro Opportunità di risparmio del dashboard Governance, seleziona *Ottimizza storage* per visualizzare i risultati filtrati nella pagina Investigation. Per scoprire eventuali dati che dovresti eliminare o spostare su storage meno costoso, esamina le _Opportunità di risparmio_.
+
...
.Passi
. Da Classificazione dati, seleziona *Governance*.
. Nel riquadro dei report, seleziona il pulsante **Scarica** nell'intestazione del report Panoramica completa della mappatura dei dati.
+
