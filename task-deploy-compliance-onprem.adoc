...
summary: 'Per installare NetApp Data Classification su un host Linux nella tua rete o su un host Linux nel cloud con accesso a Internet, devi distribuire manualmente l"host Linux nella tua rete o nel cloud.' 
---
= Installare NetApp Data Classification su un host con accesso a Internet
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Per installare e utilizzare NetApp Data Classification è necessario un agente Console. Nella maggior parte dei casi, probabilmente avrai già configurato un agente Console prima di tentare di attivare NetApp Data Classification.
...
Nella maggior parte dei casi, installi Data Classification su un singolo sistema host. Questo non è lo stesso host VM dell'agente Console. <<Installazione a host singolo per configurazioni tipiche,Guarda i passaggi qui>>.
...
.Passi
. Scarica il software di classificazione dei dati da https://mysupport.netapp.com/site/products/all/details/cloud-data-sense/downloads-tab/["Sito di supporto NetApp"^]. Seleziona il file denominato *DATASENSE-INSTALLER-<version>.tar.gz*.
