...
* Le scansioni *solo mappatura* forniscono solo una high-level panoramica dei dati. La mappatura può essere eseguita molto rapidamente sulle origini dati perché non accede ai file per vedere i dati all'interno.
* Le scansioni *Mappa e classifica* forniscono una scansione approfondita dei tuoi dati.


Potrebbe essere utile utilizzare le scansioni solo mappa per analizzare rapidamente i dati e identificare le origini dati che richiedono ulteriori ricerche, per poi abilitare in seguito le scansioni mappa e di classificazione solo su quelle origini dati o volumi necessari.
