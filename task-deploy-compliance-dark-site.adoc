...
---
= Installa NetApp Data Classification su un host Linux in modalità privata
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Usa la modalità privata per installare NetApp Data Classification su un host Linux in una sede on-premises senza accesso a internet (nota anche come installazione offline o dark site). Questa distribuzione utilizza uno script di installazione e mantiene l'istanza di Data Classification disconnessa dal layer SaaS di NetApp Console, permettendoti di analizzare le origini dati locali in un ambiente isolato.
...
Lo script di installazione di Data Classification verifica se il sistema e l'ambiente soddisfano i prerequisiti richiesti. Se li soddisfano, l'installazione inizia. Se vuoi verificare i prerequisiti indipendentemente dall'esecuzione dell'installazione di Data Classification, puoi scaricare un pacchetto software separato che testa solo i prerequisiti. link:task-test-linux-system.html["Scopri come verificare se il tuo host Linux è pronto per installare Data Classification"].
...
Quando installi Data Classification in modalità privata, puoi analizzare solo i dati provenienti da origini dati che sono anch'esse locali al sito on-premises. Al momento, Data Classification può analizzare le seguenti origini dati *locali*:
...
La migrazione dell'agente Console a un host diverso non è supportata quando Data Classification è distribuito su un agente Console. Se hai bisogno di Classification su un altro agente Console in un sito offline, distribuisci un nuovo agente Console e una nuova istanza di Data Classification sul nuovo host.
...
* La classificazione dei dati deve essere su un host dedicato. L'host non può essere condiviso con altre applicazioni o software di terze parti come antivirus.
...
** *Dimensioni della macchina virtuale Azure*: "Standard_D16_v5". link:reference-instance-types.html#azure-instance-types["Visualizza altri tipi di istanze di Azure"^].
...
* *Red Hat Subscription Management*: L'host deve essere registrato con Red Hat Subscription Management. Se non è registrato, il sistema non può accedere ai repository per aggiornare il software di terze parti necessario durante l'installazione.
...
NOTE: Il programma di installazione di Data Classification riserva UID 4321 e GID 4321. Prima di iniziare l'installazione, assicurati che UID 4321 e GID 4321 non siano in uso sulla macchina host. Se sono già in uso, l'installazione fallirà con un messaggio di errore "invalid user".
...
Nella maggior parte dei casi, installi Data Classification su un singolo sistema host. Questo non è lo stesso host VM dell'agente Console.

image:diagram_deploy_onprem_single_host_no_internet.png["Un diagramma che mostra la posizione delle origini dati che puoi analizzare quando utilizzi una singola istanza di Data Classification distribuita in locale senza accesso a internet."]
