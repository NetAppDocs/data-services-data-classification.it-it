...
---
= Analizza i dati nella tua organizzazione con NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
La dashboard Data Investigation visualizza informazioni dettagliate a livello di file e directory sui dati, consentendo di ordinare e filtrare i risultati. La pagina Data Investigation presenta informazioni dettagliate sui metadati e le autorizzazioni di file e directory, oltre a identificare i file duplicati. Grazie alle informazioni dettagliate a livello di file, directory e database, è possibile intraprendere azioni per migliorare la conformità dell'organizzazione e risparmiare spazio di storage. La pagina Data Investigation supporta anche link:task-copy-move.html["spostamento, copia ed eliminazione di file"].


NOTE: Per ottenere informazioni dettagliate dalla pagina Analisi, è necessario eseguire una scansione di classificazione completa delle origini dati. Le origini dati sottoposte a una scansione solo a livello di mappa non mostrano i dettagli a livello di file.
...
| Numero di identificatori | Seleziona l'intervallo di identificatori sensibili rilevati per file. Include dati personali e dati personali sensibili. Quando filtri nelle directory, NetApp Data Classification somma le corrispondenze di tutti i file in ogni cartella (e sottocartella). NOTA: la release di dicembre 2023 (versione 1.26.6) ha rimosso l'opzione per calcolare il numero di dati di identificazione personale (PII) per directory. 
...
TIP: Dopo aver filtrato i dati, puoi link:task-copy-move.html["copia, sposta ed elimina file non strutturati"] per migliorare la conformità o ottimizzare lo storage.
...
* link:task-copy-move.html["Copia, sposta ed elimina dati dalla dashboard dell'indagine"]
